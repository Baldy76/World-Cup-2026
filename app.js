...
function getWinProb(h, a) {
    if(!h || !a || h==='TBD' || a==='TBD') return { h: 33, d: 34, a: 33 };
...
// --- V12.4 BULLETPROOF DATA ENGINE ---
async function fetchAllData() {
    clearTimeout(fetchTimeout); 
    const ind = document.getElementById('api-indicator');
    ind.className = "w-3 h-3 rounded-full bg-yellow-400 animate-pulse shadow-lg"; // Loading state
    
    let parsedStandings = [];
    let parsedScorers = [];

    // 1. ISOLATED FETCH: ESPN Matches
    try {
        const mRes = await fetch(ESPN_MATCHES_URL); 
        if (!mRes.ok) throw new Error("ESPN Matches failed");
        const mData = await mRes.json();
        
        // Optional Chaining (?.) prevents fatal crashes on missing data
        globalMatches = (mData.events || []).map(e => {
            ...
            let status = 'TIMED'; 
            if (e.status?.type?.state === 'in') status = 'IN_PLAY'; 
            else if (e.status?.type?.state === 'post') status = 'FINISHED';
            
            let stage = 'GROUP_STAGE'; let group = ''; 
            const nL = (e.name || e.shortName || '').toLowerCase();
            if (nL.includes('group')) { 
                stage = 'GROUP_STAGE'; 
                const match = nL.match(/group\s+([a-z])/); 
                if(match) group = 'GROUP_' + match[1].toUpperCase(); 
            } 
            else if (nL.includes('16')) stage = 'ROUND_OF_16'; 
            else if (nL.includes('quarter')) stage = 'QUARTER_FINALS'; 
            else if (nL.includes('semi')) stage = 'SEMI_FINALS'; 
            else if (nL.includes('final')) stage = 'FINAL';
            
            return { 
                id: e.id, utcDate: e.date, status: status, stage: stage, group: group, 
                venue: comp.venue?.fullName || 'Stadium TBD', 
                homeTeam: { name: home.team?.displayName || 'TBD', tla: home.team?.abbreviation || 'TBD', crest: home.team?.logo || '' }, 
                awayTeam: { name: away.team?.displayName || 'TBD', tla: away.team?.abbreviation || 'TBD', crest: away.team?.logo || '' }, 
                score: { fullTime: { home: parseInt(home.score)||0, away: parseInt(away.score)||0 } } 
            };
        });
        ind.className = "w-3 h-3 rounded-full bg-green-400 shadow-[0_0_10px_rgba(74,222,128,0.8)]";
    } catch (e) { 
        console.error("Match Fetch Error:", e);
        ind.className = "w-3 h-3 rounded-full bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.8)]"; 
    }

    // 2. ISOLATED FETCH: ESPN Standings
    try {
        const stRes = await fetch(ESPN_STANDINGS_URL); 
        if (stRes.ok) {
            const stData = await stRes.json();
            if(stData.children) {
                parsedStandings = stData.children.map(g => ({ 
                    type: 'TOTAL', group: (g.name || 'GROUP').toUpperCase(), 
                    table: (g.standings?.entries || []).map((t, i) => ({ 
                        position: i + 1, 
                        team: { name: t.team?.displayName || 'TBD', tla: t.team?.abbreviation || 'TBD', crest: t.team?.logos?.[0]?.href || '' }, 
                        playedGames: t.stats?.find(s=>s.name==='gamesPlayed')?.value || 0, 
                        goalDifference: t.stats?.find(s=>s.name==='pointDifferential')?.value || 0, 
                        points: t.stats?.find(s=>s.name==='points')?.value || 0 
                    })) 
                }));
            }
        }
    ...
    // 3. ISOLATED FETCH: CF Proxy Scorers
    try {
    ...
    } catch(e) { console.error("Scorers Error:", e); }
    ...
    try {
        ...
        renderMatches(); 
        renderPredictor(); 
        renderStandings(parsedStandings); 
        renderBracket(); 
        renderScorers(parsedScorers); 
        populateTeamSelector();
    } catch (e) { console.error("Render Crash Prevented:", e); }
...
function evalPredictions() {
    ...
    for (let id in predictions) {
        ...
        if (match && match.status === 'FINISHED') {
            ...
            let base = 0;
            if (aH !== undefined && aH === pH && aA === pA) base = 5; 
            else if (aH !== undefined && ((aH-aA > 0 && pH-pA > 0) || (aH-aA < 0 && pH-pA < 0) || (aH-aA === 0 && pH-pA === 0))) base = 2;
    ...
    for (let id in predictions) {
        ...
        if (final !== null && match && match.stage === 'GROUP_STAGE' && match.group) {
            ...
            if (gM && gM.length > 0 && gM.every(m => m.status === 'FINISHED' && predictions[m.id] && predictions[m.id].basePoints === 5)) final *= 2; 
        }
...
async function fetchLeagueTable() {
    ...
    try {
        ...
        if (!data || !data.length) return container.innerHTML = `<div class="p-6 text-center opacity-40 text-[10px] uppercase font-black">No party guests yet.</div>`;
...
function getTvBadge(h, a) { if(!h||!a) return ''; const channel = UK_TV_GUIDE[`${h}-${a}`] || UK_TV_GUIDE[`${a}-${h}`]; if (!channel) return ''; const style = channel === "BBC" ? "bg-black text-white" : "bg-blue-900 text-cyan-300 border-cyan-300/30"; return `<span class="${style} px-2 py-0.5 rounded-full text-[8px] font-black tracking-widest ml-2 border border-white/10 shadow-sm">${channel}</span>`; }
...
function renderMatches() {
    ...
    if (!list || !list.length) return container.innerHTML = `<div class="text-center py-20 opacity-40 font-black uppercase text-[10px] tracking-widest">No matches found or Schedule pending.</div>`;
                        ...
                        return `
                            <div onclick="toggleDetails(${i})" class="glass p-6 rounded-3xl shadow-lg border-l-4 ${isLive ? 'border-red-500 shadow-[0_0_15px_rgba(239,68,68,0.5)]' : 'border-transparent'} active:scale-95 transition">
                                ...
                                <div class="flex justify-between items-center text-2xl font-black italic tracking-tighter">
                                    <div class="flex-1 flex items-center justify-end truncate pl-2 pr-2 text-right drop-shadow-md">${m.homeTeam?.tla || 'TBD'} <img src="${m.homeTeam?.crest}" class="w-6 h-6 ml-3 object-contain inline drop-shadow-md" onerror="this.style.display='none'"></div>
                                    ...
                                    <div class="flex-1 flex items-center justify-start truncate pl-2 text-left drop-shadow-md"><img src="${m.awayTeam?.crest}" class="w-6 h-6 mr-3 object-contain inline drop-shadow-md" onerror="this.style.display='none'"> ${m.awayTeam?.tla || 'TBD'}</div>
                                </div>
...
function renderPredictor() {
    ...
    if (!u || !u.length) return c.innerHTML = `<div class="glass p-6 rounded-3xl text-center opacity-50 font-black text-xs uppercase tracking-widest">No upcoming matches.</div>`;
    let html = ""; let cD = "";
    u.forEach(m => {
        ...
        html += `<div class="glass p-6 rounded-3xl shadow-lg border border-white/10 mb-5"><div class="flex justify-between items-center mb-4"><span class="stage-tag shadow-sm">${m.group?'GROUP '+m.group.split('_')[1]:m.stage.replace('_',' ')}</span><span class="text-[10px] font-black opacity-50 tracking-widest">${new Date(m.utcDate).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</span></div><div class="flex items-center justify-between"><div class="flex flex-col items-center w-1/3"><img src="${m.homeTeam?.crest}" class="w-8 h-8 mb-2 object-contain drop-shadow-md" onerror="this.style.display='none'"><span class="text-xs font-black uppercase truncate drop-shadow-md px-1">${m.homeTeam?.tla || 'TBD'}</span></div><div class="flex space-x-3 items-center w-1/3 justify-center"><input id="pred-h-${m.id}" type="number" min="0" value="${p?p.h:''}" class="w-12 h-12 bg-white/20 text-center font-black rounded-xl outline-none border border-white/20 shadow-inner text-lg"><span class="opacity-50 font-black text-xl">-</span><input id="pred-a-${m.id}" type="number" min="0" value="${p?p.a:''}" class="w-12 h-12 bg-white/20 text-center font-black rounded-xl outline-none border border-white/20 shadow-inner text-lg"></div><div class="flex flex-col items-center w-1/3"><img src="${m.awayTeam?.crest}" class="w-8 h-8 mb-2 object-contain drop-shadow-md" onerror="this.style.display='none'"><span class="text-xs font-black uppercase truncate drop-shadow-md px-1">${m.awayTeam?.tla || 'TBD'}</span></div></div><button id="btn-save-${m.id}" onclick="savePrediction('${m.id}', '${m.homeTeam?.name || 'TBD'}', '${m.awayTeam?.name || 'TBD'}')" class="w-full mt-6 py-3 gradient-btn font-black text-xs uppercase tracking-widest rounded-2xl active:scale-95 transition shadow-lg">${p ? 'Update Pick' : 'Lock It In 🔒'}</button></div>`;
    });
        ...
        <div class="flex justify-between items-center py-4 px-2 border-b border-white/10 last:border-0">
            <div class="flex items-center gap-4">
                ...
                <div class="flex flex-col">
                    <span class="font-black text-sm tracking-wide drop-shadow-sm">${player.player?.name || 'Unknown'}</span>
                    <span class="text-[9px] font-black uppercase tracking-widest opacity-60">${player.team?.name || ''}</span>
                </div>
            </div>
            <span class="font-black text-xl primary-text drop-shadow-md">${player.goals || 0} ⚽</span>
        </div>
    `).join('') + `</div>`;
}

function renderBracket() { const c = document.getElementById('sub-bracket'); const k = globalMatches.filter(m => m.stage !== 'GROUP_STAGE' && m.stage !== null); if(!k || !k.length) return c.innerHTML = `<div class="text-center py-20 opacity-40 font-black uppercase tracking-widest text-[10px]">Bracket arriving soon.</div>`; const stages = { 'ROUND_OF_32':[], 'ROUND_OF_16':[], 'QUARTER_FINALS':[], 'SEMI_FINALS':[], 'FINAL':[] }; k.forEach(m => { if(stages[m.stage]) stages[m.stage].push(m); }); let html = `<div class="flex space-x-6 px-4">`; Object.keys(stages).forEach(n => { if(!stages[n].length) return; html += `<div class="flex flex-col space-y-4 min-w-[180px] justify-around"><h4 class="text-center text-[10px] font-black uppercase opacity-60 mb-3 tracking-widest">${n.replace('_',' ')}</h4>`; stages[n].forEach(m => { const hS = m.score?.fullTime?.home ?? '-'; const aS = m.score?.fullTime?.away ?? '-'; html += `<div class="glass p-3 rounded-2xl text-xs font-black border-l-4 ${m.status==='IN_PLAY'?'border-red-500 shadow-[0_0_10px_rgba(239,68,68,0.3)]':'border-white/20'}"><div class="flex justify-between items-center mb-2"><span class="truncate pr-2 drop-shadow-sm pl-1">${m.homeTeam?.tla||'TBD'}</span><span class="${hS>aS?'primary-text text-sm':''}">${hS}</span></div><div class="flex justify-between items-center"><span class="truncate pr-2 drop-shadow-sm pl-1">${m.awayTeam?.tla||'TBD'}</span><span class="${aS>hS?'primary-text text-sm':''}">${aS}</span></div></div>`; }); html += `</div>`; }); html += `</div>`; c.innerHTML = html; }
