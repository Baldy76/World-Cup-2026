...
// Proxy Rotator: If one fails, we automatically try the next.
const PROXIES = [
    'https://corsproxy.org/?',
...
// --- CUSTOM FETCH TO HANDLE FLAKY PROXIES ---
async function fetchWithProxy(targetUrl, options) {
    for (let proxy of PROXIES) {
        try {
            // Format URL based on what the proxy expects
            ...
            // If it's a successful response OR a deliberate API rate limit, return it
            if (response.ok || response.status === 429) {
                return response; 
            }
            console.warn(`Proxy failed with status ${response.status}. Trying next...`);
        } catch (err) {
            console.warn(`Proxy network error. Trying next...`);
        }
    }
    throw new Error("All CORS proxies failed to connect.");
}
...
async function fetchMatches() {
    ...
    try {
        // Use our new resilient fetch function
        ...
        // 4. Smart Throttling for next fetch
        ...
        console.error(error);
        statusMsg.textContent = "Proxies overloaded. Retrying in 60s...";
