// --- V11 ESPN PIPELINE ---
const CF_API_URL = 'https://wc2026-proxy.baldynapperrwe.workers.dev'; // We only use CF for League DB now!
...
function fireConfetti() { if(window.confetti) { triggerHaptic('success'); confetti({ particleCount: 120, spread: 80, origin: { y: 0.6 }, zIndex: 9999, colors: ['#10b981', '#3b82f6', '#ffffff'] }); } }
...
// --- V11 ESPN TRANSLATOR ENGINE ---
async function fetchAllData() {
    clearTimeout(fetchTimeout);
    const indicator = document.getElementById('api-indicator');
    
    try {
        // 1. Fetch from ESPN directly (No API keys needed!)
        const mRes = await fetch(ESPN_MATCHES_URL);
        const mData = await mRes.json();

        // 2. Translate ESPN's crazy format into our elegant format
        globalMatches = mData.events.map(e => {
            ...
            return {
                ...
                homeTeam: { name: home.team.displayName, tla: home.team.abbreviation, crest: home.team.logo },
                awayTeam: { name: away.team.displayName, tla: away.team.abbreviation, crest: away.team.logo },
                score: { fullTime: { home: parseInt(home.score)||0, away: parseInt(away.score)||0 } }
            };
        });

        // 3. Fetch Standings from ESPN
        let parsedStandings = [];
        try {
            const stRes = await fetch(ESPN_STANDINGS_URL); const stData = await stRes.json();
            if(stData.children) {
                parsedStandings = stData.children.map(g => ({
                    type: 'TOTAL', group: g.name.toUpperCase(),
                    table: g.standings.entries.map((t, i) => ({
                        position: i + 1, team: { name: t.team.displayName, tla: t.team.abbreviation, crest: t.team.logos?.[0]?.href },
...
function checkGoalAlerts(matches) {
    matches.forEach(match => {
        if (match.status === 'IN_PLAY') {
            const currentH = match.score?.fullTime?.home ?? 0; const currentA = match.score?.fullTime?.away ?? 0;
            ...
            if (prev && (currentH > prev.home || currentA > prev.away)) {
                ...
                if (savedTeam !== 'ALL' && scoringTeam.name === savedTeam) fireConfetti();
                if (("Notification" in window) && Notification.permission === "granted") {
                    if (savedTeam === 'ALL' || match.homeTeam?.name === savedTeam || match.awayTeam?.name === savedTeam) {
                        new Notification(`⚽ GOAL ${scoringTeam.tla}!`, { body: `${match.homeTeam.tla} ${currentH} - ${currentA} ${match.awayTeam.tla}`, icon: scoringTeam.crest || '' });
                    }
                }
            }
...
async function fetchLeagueTable() {
    ...
    try {
        ...
        container.innerHTML = data.map((player, index) => {
            ...
            return `<div class="flex justify-between items-center p-4 border-b border-black/5 last:border-0 ${isMe ? 'bg-emerald-500/10' : ''}"><div class="flex items-center gap-4">${medal}<span class="font-black text-sm ${isMe ? 'text-emerald-500' : ''}">${player.username} ${badges}</span></div><span class="font-black text-xl font-mono ${isMe ? 'text-emerald-500' : ''}">${player.points}</span></div>`;
        }).join('');
    } catch(e) {}
}
...
function getTvBadge(h, a) { const channel = UK_TV_GUIDE[`${h}-${a}`] || UK_TV_GUIDE[`${a}-${h}`]; if (!channel) return ''; const style = channel === "BBC" ? "bg-black text-white" : "bg-blue-900 text-cyan-300 border-cyan-300/30"; return `<span class="${style} px-1.5 py-0.5 rounded text-[8px] font-black tracking-widest ml-2 border border-white/10 shadow-sm">${channel}</span>`; }
...
function renderMatches() {
    ...
    const groups = {}; list.forEach(m => { const dStr = new Date(m.utcDate).toLocaleDateString(undefined, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }); if(!groups[dStr]) groups[dStr] = []; groups[dStr].push(m); });

    let html = "";
    Object.keys(groups).forEach((date, index) => {
        const matches = groups[date]; const dateId = `date-${index}`;
        html += `<div class="mb-4"><div id="header-${dateId}" onclick="toggleDateGroup('${dateId}')" class="date-header glass flex justify-between items-center px-4 py-3 rounded-2xl shadow-sm border border-black/5"><div class="flex flex-col"><span class="text-[10px] font-black uppercase tracking-widest opacity-60">${date}</span><span class="text-[8px] font-bold primary-text uppercase">${matches.length} MATCH${matches.length > 1 ? 'ES' : ''}</span></div><svg class="w-4 h-4 opacity-40" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></div>
                <div id="drawer-${dateId}" class="match-drawer mt-3 space-y-4 px-1">${matches.map((m, i) => {
                        ...
                        return `<div onclick="toggleDetails(${i})" class="glass p-5 rounded-2xl shadow-md border-l-4 ${isLive ? 'border-red-500' : 'border-emerald-500'} active:scale-95 transition"><div class="flex justify-between items-center mb-3"><span class="stage-tag">${stageDisplay}</span><div class="flex items-center gap-2">${tv}<span class="text-[10px] font-bold opacity-40 uppercase">${tStr}</span></div></div><div class="flex justify-between items-center text-lg font-black italic tracking-tighter"><div class="flex-1 flex items-center justify-end truncate pr-2 text-right">${m.homeTeam?.tla || 'TBD'} <img src="${m.homeTeam?.crest}" class="w-5 h-5 ml-2 object-contain inline"></div><div class="px-3 py-1 bg-black/5 rounded-lg font-mono">${m.score.fullTime.home ?? 0} - ${m.score.fullTime.away ?? 0}</div><div class="flex-1 flex items-center justify-start truncate pl-2 text-left"><img src="${m.awayTeam?.crest}" class="w-5 h-5 mr-2 object-contain inline"> ${m.awayTeam?.tla || 'TBD'}</div></div>
                                <div id="details-${i}" class="hidden mt-4 pt-4 border-t border-black/5"><div class="text-[8px] font-black uppercase tracking-widest opacity-40 text-center mb-1">Win Probability</div><div class="w-full h-1.5 flex rounded-full overflow-hidden opacity-80 mb-1"><div style="width: ${prob.h}%" class="bg-blue-500"></div><div style="width: ${prob.d}%" class="bg-slate-400"></div><div style="width: ${prob.a}%" class="bg-red-500"></div></div><div class="flex justify-between text-[7px] font-black uppercase tracking-widest opacity-40 mb-3"><span>${prob.h}%</span><span>DRAW ${prob.d}%</span><span>${prob.a}%</span></div><div class="text-[9px] text-center opacity-40 font-bold uppercase tracking-widest">${m.venue || 'Stadium TBD'}</div></div></div>`;
                    }).join('')}</div></div>`;
    });
...
function renderPredictor() {
    ...
    if(past.length) {
        ...
        past.forEach(id => {
            const p = predictions[id]; const color = p.points >= 5 ? 'emerald' : p.points >= 2 ? 'blue' : 'slate'; const bonus = (p.points > p.basePoints) ? `<span class="bg-emerald-500 text-white text-[7px] px-1 rounded ml-1 font-black">DOUBLE</span>` : '';
...
function renderStandings(s) { const c = document.getElementById('sub-groups'); if (!s || !s.length) return c.innerHTML = `<div class="text-center py-20 opacity-40 font-bold uppercase text-[10px]">No ESPN Standings data yet.</div>`; c.innerHTML = s.filter(g => g.type === 'TOTAL').map(group => `<div class="glass p-4 rounded-2xl shadow-md mb-4"><h3 class="font-black primary-text mb-3 border-b border-black/5 pb-2 text-[10px] uppercase">${group.group.replace('_',' ')}</h3><div class="space-y-2">${group.table.map(team => `<div class="flex justify-between items-center text-[10px]"><div class="flex items-center gap-2 w-1/2"><span class="opacity-40 w-3">${team.position}</span><img src="${team.team.crest}" class="w-4 h-4 object-contain"><span class="font-bold truncate">${team.team.tla}</span></div><div class="flex gap-3 font-mono opacity-80 w-1/2 justify-end"><span>${team.playedGames}P</span><span class="font-black text-emerald-500">${team.points}pts</span></div></div>`).join('')}</div></div>`).join(''); }
function renderScorers(s) { const c = document.getElementById('tab-stats'); c.innerHTML = `<h2 class="text-xl font-black primary-text mb-4 text-center">GOLDEN BOOT</h2><div class="glass p-5 rounded-2xl text-center opacity-40 text-[10px] uppercase py-10">Data will map after Kickoff</div>`; }
